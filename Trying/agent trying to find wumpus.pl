...
addtostenchlist(X) :-
    nb_getval(stenchlist, L),
    append(L, X, NL),
    nb_setval(stenchlist, NL).

getstenchlist :-
    nb_delete(stenchlist),
    nb_setval(stenchlist, []),
    forall(stench(X, Y), addtostenchlist([[X,Y]])).
...
getwumpuslocation(L) :-
    ...
    StartY1 is StartY-1,
    StartY2 is StartY+1,
    ((StartX1 = EndX; StartX2 = EndX) 
    ->  (not(safe(EndX,StartY)) 
        ->  write(assertzA),
            assertz(wumpus(EndX,StartY))
        ; !)
    ;   (StartY1 = EndY; StartY2 = EndY) 
    ->  (not(safe(StartX,EndY)) 
        ->  write(assertzB),
            assertz(wumpus(StartX,EndY))
        ; !)
    ;   !).
